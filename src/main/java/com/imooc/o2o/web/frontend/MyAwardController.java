...
public class MyAwardController {
	...
	@RequestMapping(value="getawardbyawardid")
	@ResponseBody
	public Map<String, Object> getAwardByAwardId(HttpServletRequest request){
		Map<String,Object> modelMap = new HashMap<String, Object>();
	...
	/**
	 * 生成奖品的领取二维码，供操作员扫描，证明已领取，微信扫一扫就能链接对应的URL
	 * 
	 * @param request
	 * @param response
	 */
	...
	private void generateQRCode4Award(HttpServletRequest request, HttpServletResponse response) {
		...
		UserAwardMap userAwardMap = userAwardMapService.getUserAwardMapById(userAwardId);
		PersonInfo user = (PersonInfo) request.getSession().getAttribute("user");
		if (userAwardMap != null && userAwardMap.getUser().getUserId() == user.getUserId() && user != null
				&& user.getUserId() != null) {
			// 获取时间戳，以保证二维码的时间有效性，精确到毫秒
			long timpStamp = System.currentTimeMillis();
			// 将顾客奖品映射id，顾客id和timeStamp传入content，赋值到state中，这样微信获取到这些信息后会回传到授权信息的添加方法
