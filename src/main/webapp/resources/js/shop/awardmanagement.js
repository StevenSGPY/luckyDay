$(function() {
	...
	var deleteUrl = '/o2o/shopadmin/modifyaward';
	getList();
	function getList() {
			...
			if (data.success) {
				...
				awardList.map(function(item, index) {
					...
					if (item.enableStatus == 0) {
						//若状态值为0，表明是已下架的奖品，操作变为上架（即点击上架按钮上架相关）
						textOp = "上架";
						contraryStatus = 1;
					} else {
						contraryStatus = 0;
			...
			.on(
					...
					function(e) {
						...
						if (target.hasClass('edit')) {
							...
							//如果是预览，跳转到前台页面预览商品详情 TODO 暂无前台页面
							window.location.href="/o2o/frontend/myawarddetail?awardId="+e.currentTarget.dataset.id;
						}
					});
